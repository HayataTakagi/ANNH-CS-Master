...
The result of changing $\alpha$ is shown in Figure \ref{fig:alpha}. For all methods, when $\alpha$ approaches 1, the processing time become large. The point-base method and the mbr-based method in all experiments which $\alpha$ is greater than 0.6, and the grid-based method in the MAX experiments which $\alpha$ is greater than 0.7 are omitted because the processing time become too large. As $\alpha$ becomes larger, the filtering function do not work well and the processing time increase. In the NE dataset and the CAS dataset, quadtree-based method is the fastest and has the slowest increase in processing time. In the UN dataset, the grid-based method is faster than the quadtree-based method when $\alpha$ is lager than 0.5 because the answer clusters are more likely to be near the centroid of query.
