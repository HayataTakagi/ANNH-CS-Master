...
def main():
    ...
    ax = fig.add_subplot(321, xlabel='k', ylabel='Processing time (ms)', yscale='log')
    ...
    k_ne_max_data_ = k_ne_max_data / (1000 * test_size)  # μsからmsへの変換
    ax2 = fig.add_subplot(322, xlabel='k', ylabel='Processing time (ms)', yscale='log')
    ...
    k_cas_sum_data_ = k_cas_sum_data / (1000 * test_size)  # μsからmsへの変換
    ax3 = fig.add_subplot(323, xlabel='k', ylabel='Processing time (ms)', yscale='log')
    ...
    k_cas_max_data_ = k_cas_max_data / (1000 * test_size)  # μsからmsへの変換
    ax4 = fig.add_subplot(324, xlabel='k', ylabel='Processing time (ms)', yscale='log')
    ...
    k_un_sum_data = np.loadtxt('data/K-UN-SUM.csv', skiprows=1, delimiter=',')
    k_un_sum_data_ = k_un_sum_data / (1000 * test_size)  # μsからmsへの変換
    ax5 = fig.add_subplot(325, xlabel='k', ylabel='Processing time (ms)', yscale='log')
    ax5.plot(k_un_sum_data[:, 0], k_un_sum_data_[:, 1], marker='x')
    ax5.plot(k_un_sum_data[:, 0], k_un_sum_data_[:, 2], marker='s')
    ax5.plot(k_un_sum_data[:, 0], k_un_sum_data_[:, 3], marker='o')
    ax5.plot(k_un_sum_data[:, 0], k_un_sum_data_[:, 4], marker='^')
    ax5.set_title('(e) UN,SUM', loc='center', y=-0.27, fontweight='bold')
    # ax5.set_ylim(10, 1000000)

    k_un_max_data = np.loadtxt('data/K-UN-MAX.csv', skiprows=1, delimiter=',')
    k_un_max_data_ = k_un_max_data / (1000 * test_size)  # μsからmsへの変換
    ax6 = fig.add_subplot(326, xlabel='k', ylabel='Processing time (ms)', yscale='log')
    ax6.plot(k_un_max_data[:, 0], k_un_max_data_[:, 1], marker='x')
    ax6.plot(k_un_max_data[:, 0], k_un_max_data_[:, 2], marker='s')
    ax6.plot(k_un_max_data[:, 0], k_un_max_data_[:, 3], marker='o')
    ax6.plot(k_un_max_data[:, 0], k_un_max_data_[:, 4], marker='^')
    ...
    fig.savefig('export/clustersize2.pdf', facecolor=fig.get_facecolor(), edgecolor=fig.get_edgecolor())
