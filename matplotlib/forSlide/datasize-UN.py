...
def main():
    ...
    ds_ne_sum_data = np.loadtxt('../data/DS-UN-SUM.csv', skiprows=1, delimiter=',')
    ds_ne_sum_data_ = ds_ne_sum_data / 1000  # μsからmsへの変換
    ...
    ds_ne_max_data = np.loadtxt('../data/DS-UN-MAX.csv', skiprows=1, delimiter=',')
    ds_ne_max_data_ = ds_ne_max_data / 1000  # μsからmsへの変換
    ...
    fig.savefig('export/datasize-UN.pdf', facecolor=fig.get_facecolor(), edgecolor=fig.get_edgecolor())
