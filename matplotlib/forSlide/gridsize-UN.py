...
def main():
    ...
    gs_ne_sum_data = np.loadtxt('../data/GS-UN-SUM.csv', skiprows=1, delimiter=',')
    gs_ne_sum_data_ = gs_ne_sum_data / 10  # μsからmsへの変換
    ...
    gs_ne_max_data = np.loadtxt('../data/GS-UN-MAX.csv', skiprows=1, delimiter=',')
    gs_ne_max_data_ = gs_ne_max_data / 10  # μsからmsへの変換
    ...
    fig.savefig('export/gridsize-UN.pdf', facecolor=fig.get_facecolor(), edgecolor=fig.get_edgecolor())
